$(window).on('load', function () {
	...
	if ($(window).width() < 767) {
		...
		setTimeout(function() {
			$("head").append('<link rel="preconnect" href="https://db.onlinewebfonts.com">');
			$("head").append('<link href="https://db.onlinewebfonts.com/c/9d4d315a044f24c1f1654fa90df10d13?family=Pistoletto-Black" rel="stylesheet">');
...
$(function(){
	...
	/* CONTROLES DE BOTONES - SECCION DESTACADOS */
	$(document).ready(function () {
		// Función para abrir el enlace del producto en una nueva pestaña
		$('.product-link').on('click', function () {
			// Obtiene el enlace del atributo data-url
		...
		$('.whatsapp-link').on('click', function () {
			...
			const whatsappNumber = '573103624742';
			// Construye la URL de WhatsApp con el mensaje personalizado
	...
	const phoneNumber = "+573103624742";
...
$(document).ready(function () {
    ...
    // Remove any previously attached events to avoid duplicates
    $(".counter-link__prev").off("click");
    $(".counter-link__next").off("click");
    ...
    $("#buyButton").on("click", function (event) {
        ...
        // Get the quantity and product title
        let quantity = $("#quantity").val();
        let productTitle = $(".product-title").text().trim();

        // Construct the WhatsApp message
        let message = `Buen día, vengo de la página de Choe y me encuentro interesado en adquirir el producto "${productTitle}" en una cantidad de ${quantity} unidades.`;
...
const slider = document.querySelector(".sus-slider");
const leftSlides = slider.querySelectorAll(".sus-left > div");
const rightSlides = slider.querySelectorAll(".sus-right > div");
const upButton = slider.querySelector(".sus-controls .sus-up");
const downButton = slider.querySelector(".sus-controls .sus-down");

let currentSlide = 0;
const totalSlides = leftSlides.length;

function updateSlides() {
  // Mueve los contenedores hacia arriba o abajo
